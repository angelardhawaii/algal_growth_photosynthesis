...
ulva %>% ggplot(aes(treatment_graph, rETRmaxYpoint1)) + 
  ...
  labs(x="salinity/nitrate", y= "Day 9 rETRmax (μmols electrons m-2 s-1)", title= "A", subtitle = "Ulva lactuca") + 
...
hypnea %>% ggplot(aes(treatment_graph, rETRmaxYpoint1)) + 
  geom_boxplot(size=0.5) + 
  geom_point(alpha = 0.75, size = 3, aes(color = Temperature), show.legend = TRUE) + 
  labs(x="Treatment", y= "Day 9 rETRmax (μmols electrons m-2 s-1)", title= "B", subtitle = "Hypnea musciformis") + 
...
hypnea_etr_ek_graph <- ggplot(hypnea, aes(x=rETRmaxYpoint1, y=ek.1)) + 
  geom_point(alpha = 0.5, size = 3, show.legend = TRUE, aes(color = Treatment)) + 
  geom_smooth(method = "lm", col = "black") + theme_bw() + 
...
bartlett.test(rETRmax ~ Temperature, data = ulva)
...
bartlett.test(rETRmax ~ Temperature, data = hypnea)
