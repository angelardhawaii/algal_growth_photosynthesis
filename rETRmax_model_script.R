...
ulva_growth_retrmax_model <- lmer(formula = growth_rate ~ rETRmax + Treatment + (1 | Run) + (1 | Plant.ID), data = ulva)
...
performance ::check_model(ulva_growth_retrmax_model)
r.squaredGLMM(all_runs_photosyn_model_noint)
summary(all_runs_photosyn_model_noint)
r.squaredGLMM(ulva_growth_retrmax_model)
...
ulva_growth_ek_model <- lmer(formula = growth_rate ~ ek.1 + Treatment + (1 | Run) + (1 | Plant.ID), data = ulva)
...
performance::check_model(ulva_growth_ek_model)
r.squaredGLMM(all_runs_photosyn_model_ek)
r.squaredGLMM(ulva_growth_ek_model)
...
ulva_photo_no2.5$growth_rate <- round((ulva_growth_no2.5$final.weight - ulva_growth_no2.5$Initial.weight) / 
                                        ulva_growth_no2.5$Initial.weight * 100, digits = 2)
ulva_growth_ek_no2.5_graph <- ggplot(ulva_photo_no2.5, aes(x=ek.1, y=growth_rate)) + 
  geom_point(alpha = 0.5, size = 3, show.legend = TRUE, aes(color = Treatment)) + 
  geom_smooth(method = "lm", col = "black") + theme_bw() + 
  labs(title = "Ulva lactuca Ek vs Growth Rate", x = "Ek (μmols photons m-2 s-1)", 
       y = "growth rate (%)") + stat_regline_equation(label.x = 25, label.y = 165) + stat_cor()
ulva_growth_ek_no2.5_graph
...
all_runs_photosyn_model_hyp <- lmer(formula = rETRmax ~ Treatment + Temperature + (1 | Run) + (1 | Plant.ID) + (1 | RLC.Order), data = hypnea)
hyp_growth_retrmax_model <- lmer(formula = growth_rate ~ rETRmax + Treatment + Temperature + (1 | Run) + (1 | Plant.ID), data = hypnea)
...
performance::check_model(hyp_growth_retrmax_model)
r.squaredGLMM(all_runs_photosyn_model_hyp)
r.squaredGLMM(hyp_growth_retrmax_model)
...
hyp_growth_ek_model <- lmer(formula = growth_rate ~ ek.1 + Treatment + Temperature + (1 | Run) + (1 | Plant.ID), data = hypnea)
#make a histogram and residual plots of the data for hypnea
...
#check the performance of the model
performance::check_model(all_runs_photosyn_model_ek)
performance::check_model(hyp_growth_ek_model)

r.squaredGLMM(all_runs_photosyn_model_ek)
r.squaredGLMM(hyp_growth_ek_model)
summary(all_runs_photosyn_model_ek)
